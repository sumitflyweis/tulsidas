  ...
  try {
    ...
    if (!user) {
      ...
      let obj = {
        id: userCreate._id,
        otp: userCreate.otp,
        otpExpiration:userCreate.otpExpiration,
        accountVerification:userCreate.accountVerification,
...
exports.editProfile1 = async (req, res) => {
  try {
    const findUser = await User.findById({ _id: req.user._id });
    if (findUser) {
      const data = {
        firstName: req.body.firstName || findUser.firstName,
        middleName: req.body.middleName || findUser.middleName,
        lastName: req.body.lastName || findUser.lastName
      }
